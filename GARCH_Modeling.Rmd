...
train_data_raw <- DGS10_diff[paste0(train_start, "/", train_end)]
test_data <- na.omit(DGS10_diff["2025-01-01/"])
spec <- ugarchspec(
  variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
  mean.model = list(armaOrder = c(0, 0), include.mean = TRUE),
  distribution.model = "std"
...
key_events <- data.frame(
  COVID = ifelse(index(train_data) >= as.Date("2020-03-01") & index(train_data) <= as.Date("2021-12-31"), 1, 0),
  Trump = ifelse(index(train_data) >= as.Date("2016-11-08") & index(train_data) <= as.Date("2020-01-20"), 1, 0)
)
# Add indicators to the training data
...
fit_with_indicators <- ugarchfit(spec_with_indicators, train_data[,1])
...
forecast_orig <- ugarchforecast(fit_with_indicators, n.ahead = nrow(test_data))
# Extract forecasted volatility
forecasted_volatility <- as.numeric(forecast@forecast$sigmaFor)
